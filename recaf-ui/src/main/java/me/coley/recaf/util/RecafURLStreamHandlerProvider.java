...
public class RecafURLStreamHandlerProvider extends URLStreamHandlerProvider {
	...
	private static class ConnectionImpl extends URLConnection {
		...
		private void loadContent() throws IOException {
			...
			if (workspace == null)
				throw new IOException("Рабочее пространство не открыто!");
			// Transform path
			...
			switch (protocol) {
				case recafClass:
					...
					if (classInfo == null)
						throw new IOException("Класс не найден в текущем рабочем пространстве: " + path);
					content = classInfo.getValue();
					break;
				case recafFile:
					FileInfo fileInfo = workspace.getResources().getFile(path);
					if (fileInfo == null)
						throw new IOException("Файл не найден в текущем рабочем пространстве: " + path);
					content = fileInfo.getValue();
					break;
				default:
					throw new IOException("Неизвестный протокол: " + protocol);
			}
		}
	}
}
