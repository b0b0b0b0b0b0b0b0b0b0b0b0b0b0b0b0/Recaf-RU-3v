...
public class JFXInjection {
	...
	private static void alertUserFailedInit(Exception ex) {
		...
		showErrorMessageWindow(toolkit, errorString, "<style>" +
				...
				"<p>Произошла ошибка при загрузке JavaFX.<br>" +
						"<b>Следующая информация о проблеме была скопирована в ваш буфер обмена:</b></p><br>" +
						"<pre>" + errorString + "</pre>" +
						"<p>Пожалуйста, убедитесь, что выполняется одно из следующих условий:<br>" +
						" 1. Используется JDK, содержащий JavaFX<br>" +
						" 2. Обновитесь до Java 11 или выше <i>(Recaf автоматически загрузит JavaFX)</i><br><br>" +
						"Если вы считаете, что это ошибка, пожалуйста, " +
						"<a href=\"" + bugReportURL + "\">создайте обращение на GitHub</a></p>");
	}
