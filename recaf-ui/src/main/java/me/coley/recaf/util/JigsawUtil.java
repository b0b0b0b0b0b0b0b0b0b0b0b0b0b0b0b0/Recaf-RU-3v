...
public class JigsawUtil {
	...
	static Module getClassModule(Class<?> klass) {
		try {
			...
			logger.error("Не удалось получить модуль класса: " + klass.getName(), t);
	...
	static Module getLoaderModule(ClassLoader loader) {
		try {
			...
			logger.error("Не удалось получить модуль ClassLoader: " + loader, t);
			// That should never happen.
			throw new AssertionError(t);
		}
	}
	...
	static void setMethodModifiers(Method method, int modifiers) {
		try {
			...
			logger.error("Не удалось установить модификаторы метода: " + method.getName(), t);
			// That should never happen.
			throw new AssertionError(t);
		}
	}
