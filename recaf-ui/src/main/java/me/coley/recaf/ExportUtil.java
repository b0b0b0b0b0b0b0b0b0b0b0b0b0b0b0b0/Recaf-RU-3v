...
public class ExportUtil {
	...
	public static void write(Path path) {
		...
		if (workspace == null) {
			...
			logger.warn("Попытка экспортировать приложение, но рабочее пространство не загружено! Контекст: {}", trace);
			return;
		}
	...
	public static void write(Path path, Workspace workspace) {
		logger.info("Подготовка к записи в: {}", path);
		...
		try {
			...
			logger.error("Не удалось экспортировать рабочее пространство!", e);
		}
	}
}
